...
public class SendEmail {
	...
	@ConfigProperty(name = "doodle.organizermail")
	public void sendASimpleEmail(Carpooling carpooling, String mail)  {
		// Create a default MimeMessage object.
		System.setProperty("net.fortuna.ical4j.timezone.cache.impl", MapTimeZoneCache.class.getName());
		...
		m.setTo(attendees);
		m.setFrom(organizermail);
		m.setSubject("Place reservée avec " + carpooling.getDriver() + " - Carpooling");
		m.setHtml("Vous avez reservé un place dans covoiturage de " + carpooling.getDriver() + "<br>Le rendez-vous est prévu à " + carpooling.getDeparture_time() + " à " + carpooling.getDeparture_localisation() + ".<br>Le retour est prévu à " + carpooling.getDeparture_time() + ".<br>Il reste actuellement " + carpooling.getPlaces_restantes() + " sur " + carpooling.getPlaces() + ".");

		mailer.send(m);
		System.out.println("Email Carpooling reservation sent to " + mail + " !");
	}
