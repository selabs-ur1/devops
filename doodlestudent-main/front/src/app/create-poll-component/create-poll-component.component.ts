...
export class CreatePollComponentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pollService.getAccount().subscribe(e => {
      this.account = e;
      this.connected = (e.mail != undefined);
    });
  ...
  nextPage1(): void {
    ...
    if (this.poll.id == null) {
      ...
      this.pollService.createPoll(this.poll).subscribe(p1 => {
        ...
        if(this.connected){
          this.pollService.addPoll(this.account.mail, this.poll.slug, 1).subscribe(p1 => {
            p1
          });
        }
      });
...
//        this.poll.pollChoices.filter(pc => pc.id === evt1.extendedProps.choiceid)[0].users.push({ id: -1 });
  ...
  logout(): void {
    this.pollService.logout().subscribe(e => {
      e;
    });
  }
  ...
  sendMail(): void {
    if (this.mailToSend) {
      // Le mail n'est pas envoyé, pour l'instant cela ne fait qu'ajouter le compte du membre, s'il en a un, au poll
      this.pollService.addPoll(this.mailToSend, this.poll.slug, 0).subscribe(a => {
        a;
      });
    }
  }
