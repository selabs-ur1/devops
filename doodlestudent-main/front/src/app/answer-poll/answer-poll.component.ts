...
import { Poll, ChoiceUser, PollCommentElement, User, PollChoice, CarpoolingProposition } from '../model/model';
  ...
  ngOnInit(): void {
    ...
    this.actRoute.paramMap.subscribe(params => {
      ...
      this.pollService.getPollBySlugId(this.slugid).subscribe(p => {
        ...
        this.pollService.getCarpoolingPropositionsBySlugId(this.slugid).subscribe(cp => this.carpoolingPropositions = cp);
...
eventDragStop: (timeSheetEntry, jsEvent, ui, activeView) => {
  ...
  joinCarpooling(carpoolingId: number) : boolean {
    console.log("joinCarpooling("+carpoolingId+") - answer-poll.ts");
    const mail = this.personalInformation.mail;
      if (mail) {
        console.log("carpooling : " + carpoolingId + "; mail :"+ mail);
        this.pollService.addCarpooler(carpoolingId, mail).subscribe(r => {console.log(r); this.pollService.getCarpoolingPropositionsBySlugId(this.slugid).subscribe(cp => this.carpoolingPropositions = cp);});
        ...
        this.messageService.add(
          {
            severity: 'warn',
            summary: 'Données incomplètes',
            detail: 'Veuillez remplir une adresse email'
          });
          return false;
      }
  ...
  createReponse(): void {
    if (this.personalInformation.nom && this.personalInformation.mail &&
      ...
      console.log("createReponse() - AnswerPollComponent");
      this.pollService.updateChoice4user(cu, this.poll.id).subscribe(e => {
...
//        this.poll.pollChoices.filter(pc => pc.id === evt1.extendedProps.choiceid)[0].users.push({ id: -1 });
  ...
  proposeCarpooling(driver: string, departure_localisation: string,
    departure_time: string, arriving_time: string, places: string) : void {
      this.pollService.proposeCarpooling(driver, departure_localisation,
        departure_time, arriving_time, places, this.poll.id).subscribe(c => {this.pollService.getCarpoolingPropositionsBySlugId(this.slugid).subscribe(cp => this.carpoolingPropositions = cp);});
    }
